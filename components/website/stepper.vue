...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      let lastStepOffset = 0;

      steps.forEach((step, index) => {
        totalStepsWidth += step.offsetWidth;
        if (index === 0) {
          firstStepOffset = step.offsetWidth / 2;
        }
        if (index === steps.length - 1) {
          lastStepOffset = step.offsetWidth / 2;
        }
      });
      ...
      line.style.width = `${totalStepsWidth - firstStepOffset - lastStepOffset}px`;
    });
